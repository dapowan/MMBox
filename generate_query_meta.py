...
def generate_tags(model, tags_seed, gen_tag_meta, config, log_path, tag_ind=None):

    def sample_tags_examples(tag_num, example_num):
        if tag_ind:
            chosen_tags = [tags_seed[ind] for ind in tag_ind]
        else:
...
def main(model, query_meta_seed, gen_tag_meta, config, output_dir):
    query_tags = query_meta_seed["tags"]
    for r in range(2):
        for i in range(len(query_tags)):
            output_dir_r = os.path.join(output_dir, f"round_{r}")
            query_tags = generate_tags(model, query_tags, gen_tag_meta, config, output_dir_r, tag_ind=[i])
            query_meta_seed["tags"] = query_tags
            save_dict_to_json(query_meta_seed, os.path.join(output_dir_r, "query_meta.json"))
            print("-----------------------")
    pass
...
if __name__ == '__main__':
    ...
    output_dir = "output/tag/t1"
    config = {"gen_tag_num":1, "gen_tag_example_num": 5, "gen_tag_example_output": 20}
    query_meta_seed = load_json("dataset/query_meta_v1.json")
    gen_tag_meta = read_yaml_file("prompt/gen_tag_example.yaml")
    main(model, query_meta_seed, gen_tag_meta, config, output_dir)